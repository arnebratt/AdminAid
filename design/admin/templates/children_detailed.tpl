<div class="content-navigation-childlist">
    ...
    {let child_name=$Nodes.item.name|wash
        ...
        {* ADMINAID FIX START *}
        {* Creator / Modifier *}
        <td class="modifier"><a href={$Nodes.item.object.owner.main_node.url_alias|ezurl}>{$Nodes.item.object.owner.name|wash}</a><br /><a href={$Nodes.item.object.current.creator.main_node.url_alias|ezurl}>{$Nodes.item.object.current.creator.name|wash}</a></td>

        {* Created / Modified *}
        <td class="modified">{$Nodes.item.object.published|l10n( shortdatetime )}<br />{$Nodes.item.object.modified|l10n( shortdatetime )}</td>
        {* ADMINAID FIX END *}
